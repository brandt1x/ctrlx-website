document.addEventListener('DOMContentLoaded', () => {
	...
	const VALID_PROMOS = Object.keys(PROMO_RULES);
	...
	function matchesRequiredBundle(rule, items) {
		if (!rule?.requiredProductIds?.length) return true;
		const cartIds = (items || []).map((item) => String(item?.productId || '').toLowerCase()).sort();
	...
	function getPromoEvaluation(code, items, subtotal, subscriptionCart) {
		const normalized = code && String(code).toUpperCase().trim();
		...
		if (subscriptionCart) {
			return { valid: false, error: 'Promo codes do not apply to subscription checkout.' };
		}
		if (!matchesRequiredBundle(rule, items)) {
			return { valid: false, error: 'BUNDLEXP requires VISION+X, AIM-X, and 2K Zen Script in the same cart.' };
		}

		const rawDiscount = rule.type === 'fixed'
			? rule.value
			: subtotal * (1 - (rule.value ?? 1));
		const discountAmount = Math.max(0, Math.min(subtotal, rawDiscount));
		const message = rule.type === 'fixed'
			? `$${discountAmount.toFixed(0)} off bundle applied`
			: `${Math.round((1 - (rule.value ?? 1)) * 100)}% off applied`;
	...
	function isPromoValid(code) {
		...
		return getPromoEvaluation(code, cartItems, subtotal, isSubscriptionCart()).valid;
	}
	...
	function renderSummary() {
		...
		const promoEval = getPromoEvaluation(currentPromo, items, subtotal, subscriptionCart);
		const hasDiscount = promoEval.valid && promoEval.discountAmount > 0;
		const discount = hasDiscount ? promoEval.discountAmount : 0;
	...
	function renderPromoState() {
		...
		if (currentPromo) {
			const subtotal = cartItems.reduce((sum, item) => sum + (Number(item?.price) || 0), 0);
			const promoEval = getPromoEvaluation(currentPromo, cartItems, subtotal, isSubscriptionCart());
			promoSection.innerHTML = `
				<div class="co-promo-active">
					<span class="co-promo-active-label">${currentPromo} — ${promoEval.valid ? promoEval.message : 'Promo applied'}</span>
	...
	function handleApplyPromo() {
		...
		const subtotal = cartItems.reduce((sum, item) => sum + (Number(item?.price) || 0), 0);
		const promoEval = getPromoEvaluation(code, cartItems, subtotal, isSubscriptionCart());
		if (promoEval.valid) {
			currentPromo = promoEval.code;
			try { sessionStorage.setItem(PROMO_KEY, code); } catch (_) {}
			renderSummary();
			createOrRefreshPaymentElement().catch(() => {});
		} else {
			if (msg) { msg.textContent = promoEval.error || 'Invalid promo code.'; msg.className = 'co-promo-msg co-promo-msg--error'; }
		}
	}
